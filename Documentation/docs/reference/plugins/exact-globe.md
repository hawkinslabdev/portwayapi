...
Portway provides seamless integration with Exact Globe+ through proxy endpoints, enabling external applications to interact with Globe+ data and services. This integration relies on environment-specific headers to route requests to the correct database instance.
...
1. **Authentication Failures**
   ...
   - Ensure proper environment headers

2. **Transaction Errors**
   - Review Globe+ transaction logs
   - Check for locked records
   - Verify TransactionKey consistency
...
Monitor the traffic logs for Globe+ requests:

```sql
-- Find Globe+ API errors
SELECT 
    Timestamp,
    Path,
    StatusCode,
...
FROM TrafficLogs
WHERE TargetUrl LIKE '%Exact.Entity.REST%'
